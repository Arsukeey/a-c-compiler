...
impl Type {
    ...
    pub fn get_field_ty<'a>(&'a self, field_name: &str) -> Option<&'a Type> {
        match self {
            &Type::Struct(_, ref fields) | &Type::Union(_, ref fields, _) => {
                for field in fields {
                    if let AST::VariableDecl(ref ty, ref name, _, _) = field {
                        if *name == field_name {
                            return Some(&*ty);
                        }
                    }
        ...
        match self {
            &Type::Struct(_, ref fields) | &Type::Union(_, ref fields, _) => {
                let fields_types = fields
                    .iter()
                    .map(|field| {
                        if let AST::VariableDecl(ref ty, _, _, _) = field {
                            &*ty
                        } else {
                            panic!()
                        }
    ...
    pub fn sizeof(&self) -> usize {
        match self {
            ...
            &Type::Array(ref elemty, ref size) => (*size * elemty.sizeof() as i32) as usize,
